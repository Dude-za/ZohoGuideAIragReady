...
**`zoho.encryption.urlEncode()` is NOT available in Desk.** Use manual `replaceAll` encoding — this is already applied in the code block above. The safe character set is: `\"`, `:`, `,`, ` `, `'`. Leave `{` and `}` raw — Zoho APIs accept them.
```
...
**Key lesson:** `zoho.reports.*` functions silently no-op in Desk — they return success without writing. Always use `invokeUrl` + REST for Desk → Analytics writes. In Desk, use manual `replaceAll` encoding for CONFIG parameters — `zoho.encryption.urlEncode()` is NOT available. The safe character set is: `\"`, `:`, `,`, ` `, `'`. Leave `{` and `}` raw.
